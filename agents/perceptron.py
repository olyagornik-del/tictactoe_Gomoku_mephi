...
Реализация только на NumPy. Используется для оценки позиции в
Гомоку: получает на вход 9×9 one-hot окно вокруг хода (243 признака),
выдаёт вероятность «позиция хорошая для сходившего» и так выбирает
лучший ход (см. :class:`PerceptronAgent` ниже).
